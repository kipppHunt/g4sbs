...
const double SBS_tracker_pitch=0.0*PI/180.0; //5 degrees
...
const double Ltgt = 0.4; //meters //length of the target
...
//Parameters for spin transport
int ncoeff_spin = nterms_optics;
vector<double> Cxx, Cxy, Cxz;
vector<double> Cyx, Cyy, Cyz;
vector<double> Czx, Czy, Czz;
vector<vector<int> > Cspinexpon;
...
void ElasticEP_Correlation_Analysis_withSpinTrans_julytest(const char *inputfilename, const char *outputfilename="dec9_elasticEP_output_6mmPb.root"){
  ...
  //TTree *ToutSpin = new TTree("ToutSpin","RECREATE");

  //double Pxtg, t_Pytg, t_Pztg;
  //double t_Pxfp, t_Pyfp, t_Pzfp;
  //double t_chi, t_thetaspin;
  //double t_Pxfp_eth_c, t_Pyfp_eth_c, t_Pzfp_eth_c;

  //ToutSpin->Branch("Pxtg",&Pxtg);
  ...
  TH1D *htheta_bend_temp = new TH1D("htheta_bend_temp","", 250, -90.0, 90.0);
  ...
  TH2D *hdxfp_vs_p = new TH2D("hdxfp_vs_p","",100, 0.0, 12.0, 100, -0.1,0.1);
  TH2D *hdxfp_vs_pmisse = new TH2D("hdxfp_vs_pmisse","",100, 0.0, 12.0, 100, -0.1,0.1);
  ...
  TH2D *hdxfp_vtx_known_z = new TH2D("hdxfp_vtx_known_z","",100,-0.55*Ltgt,0.55*Ltgt, 100, -0.1,0.1);
  TH2D *hdyfp_vtx_known_z = new TH2D("hdyfp_vtx_known_z","",100,-0.55*Ltgt,0.55*Ltgt, 100, -0.1,0.1);
  TH2D *hdxpfp_vtx_known_z = new TH2D("hdxpfp_vtx_known_z","",100,-0.55*Ltgt,0.55*Ltgt, 100, -0.02,0.02);
  TH2D *hdypfp_vtx_known_z = new TH2D("hdypfp_vtx_known_z","",100,-0.55*Ltgt,0.55*Ltgt, 100, -0.02,0.02);
  ...
  TH1D *hPxfpdiff = new TH1D("hPxfpdiff","",250,-0.25,0.25);
  TH1D *hPyfpdiff = new TH1D("hPyfpdiff","",250,-0.05,0.05);
  TH1D *hPzfpdiff = new TH1D("hPzfpdiff","",250,-0.05,0.05);

  TH2D *hPxfp_eth_c_vs_true = new TH2D("hPxfp_recon_vs_true","",250,-1.05,1.05,250,-1.05,1.05);
  TH2D *hPyfp_eth_c_vs_true = new TH2D("hPyfp_recon_vs_true","",250,-1.05,1.05,250,-1.05,1.05);
  TH2D *hPzfp_eth_c_vs_true = new TH2D("hPzfp_recon_vs_true","",250,-1.05,1.05,250,-1.05,1.05);
  ...
  TH1D *hPxfp_trans_true_diff = new TH1D("hPxfp_trans_true_diff","",250,-0.25,0.25);
  TH1D *hPyfp_trans_true_diff = new TH1D("hPyfp_trans_true_diff","",250,-0.05,0.05);
  TH1D *hPzfp_trans_true_diff = new TH1D("hPzfp_trans_true_diff","",250,-0.05,0.05);




  TH1D *hSxxtrue = new TH1D("hSxxtrue","",250,-1.1,1.1);
  TH1D *hSxytrue = new TH1D("hSxytrue","",250,-1.1,1.1);
  TH1D *hSxztrue = new TH1D("hSxztrue","",250,-1.1,1.1);

  TH1D *hSyxtrue = new TH1D("hSyxtrue","",250,-1.1,1.1);
  TH1D *hSyytrue = new TH1D("hSyytrue","",250,-1.1,1.1);
  TH1D *hSyztrue = new TH1D("hSyztrue","",250,-1.1,1.1);

  TH1D *hSzxtrue = new TH1D("hSzxtrue","",250,-1.1,1.1);
  TH1D *hSzytrue = new TH1D("hSzytrue","",250,-1.1,1.1);
  TH1D *hSzztrue = new TH1D("hSzztrue","",250,-1.1,1.1);
  


  TH1D *hSxx_di = new TH1D("hSxx_di","",250,-1.1,1.1);
  TH1D *hSxy_di = new TH1D("hSxy_di","",250,-1.1,1.1);
  TH1D *hSxz_di = new TH1D("hSxz_di","",250,-1.1,1.1);

  TH1D *hSyx_di = new TH1D("hSyx_di","",250,-1.1,1.1);
  TH1D *hSyy_di = new TH1D("hSyy_di","",250,-1.1,1.1);
  TH1D *hSyz_di = new TH1D("hSyz_di","",250,-1.1,1.1);

  TH1D *hSzx_di = new TH1D("hSzx_di","",250,-1.1,1.1);
  TH1D *hSzy_di = new TH1D("hSzy_di","",250,-1.1,1.1);
  TH1D *hSzz_di = new TH1D("hSzz_di","",250,-1.1,1.1);

  TH1D *hDeterminant_di = new TH1D("hDeterminant_di","",250,0.95,1.05);

  
  //TH1D *hchi_eth_chi_mc_diff = new TH1D("hchi_eth_chi_mc_diff",""250,-0.5,0.5);

  TH2D *hPxfp_eth_c_vs_chitheta = new TH2D("hPxfp_eth_c_vs_chitheta","",400,40.0,105.0,250,-1.0,0.1);
  //TH2D *hPxfp_eth_c_vs_chiphi = new TH2D("hPyfp_eth_c_vs_chiphi","",250,0.0,10.0,250,-1.0,1.0);

  TH1D *hPxfp_di = new TH1D("hPxfp_di","",250, -1.1,1.1);
  TH1D *hPyfp_di = new TH1D("hPyfp_di","",250, -1.1,1.1);
  TH1D *hPzfp_di = new TH1D("hPzfp_di","",250, -1.1,1.1);

  TH1D *hPxfp_c = new TH1D("hPxfp_c","",250, -1.1,1.1);
  TH1D *hPyfp_c = new TH1D("hPyfp_c","",250, -1.1,1.1);
  TH1D *hPzfp_c = new TH1D("hPzfp_c","",250, -1.1,1.1);

  TH1D *hPxfp = new TH1D("hPxfp","",250, -1.1,1.1);
  TH1D *hPyfp = new TH1D("hPyfp","",250, -1.1,1.1);
  TH1D *hPzfp = new TH1D("hPzfp","",250, -1.1,1.1);

  TH1D *hPxtg = new TH1D("hPxtg","",250, -1.1,1.1);
  TH1D *hPytg = new TH1D("hPytg","",250, -1.1,1.1);
  TH1D *hPztg = new TH1D("hPztg","",250, -1.1,1.1);

  TH1D *hthetaspin_eth = new TH1D("hthetaspin_eth","",250, 0.0, 100);
  TH1D *hchispin_eth = new TH1D("hchispin_eth","",250, 0.0, 100);
  ...
  double ECALtheta_default = 30.0*PI/180.0;
  double ECALdist_default = 4.7;
  double Ebeam_default = 10.6;
  ...
  while( currentline.ReadLine(infile) && !currentline.BeginsWith("endconfig") ){
	...
	if( keyword.BeginsWith("spin_transport") ){
	  spintransportfilename = ( (TObjString*) (*tokens)[1] )->GetString();
	}
      }
    }
  }
  ...
  if( spintransportfilename != ""){
    ifstream spintransportfile(spintransportfilename.Data());

    ////////cout << "MADE IT TO SPIN READ" << endl;

    while(currentline.ReadLine( spintransportfile ) ){
      if( !currentline.BeginsWith("#") ){

	double sxxtemp, sxytemp, sxztemp;
	double syxtemp, syytemp, syztemp;
	double szxtemp, szytemp, szztemp;

	vector<int> etemp(5);

	if( sscanf( currentline.Data(), " %lg %lg %lg %lg %lg %lg %lg %lg %lg %d %d %d %d %d",
		    &sxxtemp, &sxytemp, &sxztemp,
		    &syxtemp, &syytemp, &syztemp,
		    &szxtemp, &szytemp, &szztemp,
		    &(etemp[0]), &(etemp[1]), &(etemp[2]), &(etemp[3]), &(etemp[4]) ) == 14 ){
	  //cout << "GOT SPIN MATRIX ELEMENTS" << endl;
	  Cxx.push_back(sxxtemp);
	  Cxy.push_back(sxytemp);
	  Cxz.push_back(sxztemp);
	  Cyx.push_back(syxtemp);
	  Cyy.push_back(syytemp);
	  Cyz.push_back(syztemp);
	  Czx.push_back(szxtemp);
	  Czy.push_back(szytemp);
	  Czz.push_back(szztemp);

	  Cspinexpon.push_back(etemp);
	}
      }
    }	
  }
  ...
  if( ECALshowerprofile_filename != "" ){
    ...
    //cout << "MADE IT TO SHOWER PROFILE" << endl;
    ...
    */
	...
	for( int ipr=0; ipr<T->Primaries_Nprimaries; ipr++ ){
      ...
      */
      ...
      //spin variables in TRANSPORT COORDINATES
      double Pxtg = T->ev_Sx;
      double Pytg = T->ev_Sy;
      double Pztg = T->ev_Sz;
      ...
      double Pxfp = (*(T->Harm_FT_Track_Sx))[0];
      double Pyfp = (*(T->Harm_FT_Track_Sy))[0];
      double Pzfp = (*(T->Harm_FT_Track_Sz))[0];
	...
	if( Eclust[ibest] >= Eclustmin ){
	  ...
	  if( nplanes_fired == 2 ){
	    ...
	    TVector3 vertex_center_global(vertex_true_global.X(),vertex_true_global.Y(),0.0);
	    //TVector3 vertex_center_global(vertex_true_global.X(),vertex_true_global.Y(),-Ltgt/4.0);
	    ...
	    //////////////////////////////////////////////////////////////////////////////////////assuming vertex to be at upstream end of target:
	    ...
	    ///////////////////////////////////////////////////////////////////////////assuming vertex at downstream end of target:
	    ...
	    //////////////////////////////////////////////////////////////////////////////////////assuming vertex at center of target:
	    ...
	    //////////////////////////////////////////////////////////////done with center of target vertex assumption
	    ...
	    //////////////////////////////////The following proton reconstruction is seemingly all done with a central target vertex assumption//////////////////////////////////
	    ...
	    double etheta_Eclust = acos(1. - Q2_Eclust/(2.*Ebeam*Eprime_ECAL));////////////////////angle of electron based on cluster of ECal in which electron was found
	    ...
	    ////////////////////////////////////////////////comparisons of electron angle calculations to their known values from ev.data strucuture
	    ...
	    /////////////////////proton direction at target determined by electron angle
	    double xptar_eth = nhat_p_eth_SBS.X()/nhat_p_eth_SBS.Z();
	    double yptar_eth = nhat_p_eth_SBS.Y()/nhat_p_eth_SBS.Z();
	    /////////////////////assumed to be 0 since we are doing all of this assuming a central target vertex
	    double ytar_eth = 0.0;
	    double xtar_eth = 0.0;

	    /////////////////////these values will be passed by reference into the SBS_fp_recon to be calculated
	    ...
	    //////////////////// ***temp values are fits from FT tracks, so basically the real values in the simulations to which we should be comparing
	    ...
	    hdxfp_vs_p->Fill( T->ev_np, xfp_eth - xfptemp, weight );
	    ...
	    ///////////////////////////////////////////////////////END OF CENTRAL TARGET VERTEX RECONSTRUCTION OF PROTON//////////////////////////////////////////////////////SPIN AFTER THIS ACTUALLY

	    //need bend axis -> nhat_tgt cross prod with nhat_fp_global

	    //need theta bend angle -> acos(nhat_tgt dot prod with nhat_fp_global)

	    //need momentum recon precon -> pinv_sum/thetabendrecon

	    //here we are trying to get the spin transport matrix by using the expansion coeffecients; this involves building up the dipole approx and then modifying it by summing the expansion coeffecients multiplied by the reconstructed variables, first with the central target vertex assumption, so use the xpfp_eth and ypfp_eth variables, later we will use the vertex known variables(cheating) in order to compare the two spin transports and see how our reconstruction starting from electron kinematics holds up

	    /*
	    
	    TVector3 zaxis_FP_t(-sin(SBS_tracker_pitch),0,cos(SBS_tracker_pitch));
	    TVector3 yaxis_FP_t(0,1,0);
	    TVector3 xaxis_FP_t = yaxis_FP_t.Cross(zaxis_FP_t).Unit();

	    TVector3 P_tg_true = Pxtg*xaxis_FP_t + Pytg*yaxis_FP_t + Pztg*zaxis_FP_t;
	    TVector3 P_fp_true = Pxfp*xaxis_FP_t + Pyfp*yaxis_FP_t + Pzfp*zaxis_FP_t;
	    
	    //TVector3 P_tg_true(Pxtg,Pytg,Pztg);
	    //TVector3 P_fp_true(Pxfp,Pyfp,Pzfp);

	    TVector3 Spin_rot_axis_true = P_tg_true.Cross(P_fp_true).Unit();
	    
	    double angleSpinTrans = P_tg_true.Angle(P_fp_true);
	    
	    TRotation SpinTrans;
	    SpinTrans.Rotate( angleSpinTrans, Spin_rot_axis_true );
	    
	    double Sxx_true = SpinTrans.XX(), Sxy_true = SpinTrans.XY(), Sxz_true = SpinTrans.XZ();
	    double Syx_true = SpinTrans.YX(), Syy_true = SpinTrans.YY(), Syz_true = SpinTrans.YZ();
	    double Szx_true = SpinTrans.ZX(), Szy_true = SpinTrans.ZY(), Szz_true = SpinTrans.ZZ();
	    
	    TMatrixD Rot_true(3,3);
	    Rot_true(0,0) = Sxx_true; Rot_true(0,1) = Sxy_true; Rot_true(0,2) = Sxz_true;
	    Rot_true(1,0) = Syx_true; Rot_true(1,1) = Syy_true; Rot_true(1,2) = Syz_true;
	    Rot_true(2,0) = Szx_true; Rot_true(2,1) = Szy_true; Rot_true(2,2) = Szz_true;

	    TVectorD P_tg_trueD(3);
	    P_tg_trueD(0) = P_tg_true.X();
	    P_tg_trueD(1) = P_tg_true.Y();
	    P_tg_trueD(2) = P_tg_true.Z();
    
	    TVectorD P_fp_trans_true = Rot_true*P_tg_trueD;
	    //P_fp_trans_true = P_fp_trans_true.Unit();
	    
	    double Pxfp_trans_true = P_fp_trans_true(0);
	    double Pyfp_trans_true = P_fp_trans_true(1);
	    double Pzfp_trans_true = P_fp_trans_true(2);

	    TVector3 P_fp_trans_true_unit(Pxfp_trans_true, Pyfp_trans_true, Pzfp_trans_true);
	    P_fp_trans_true_unit = P_fp_trans_true_unit.Unit();

	    hPxfp_trans_true_diff->Fill(P_fp_trans_true_unit.X()-P_fp_true.X());
	    hPyfp_trans_true_diff->Fill(P_fp_trans_true_unit.Y()-P_fp_true.Y());
	    hPzfp_trans_true_diff->Fill(P_fp_trans_true_unit.Z()-P_fp_true.Z());

	    hSxxtrue->Fill( Sxx_true );
	    hSxytrue->Fill( Sxy_true );
	    hSxztrue->Fill( Sxz_true );
	    
	    hSyxtrue->Fill( Syx_true );
	    hSyytrue->Fill( Syy_true );
	    hSyztrue->Fill( Syz_true );
	    
	    hSzxtrue->Fill( Szx_true );
	    hSzytrue->Fill( Szy_true );
	    hSzztrue->Fill( Szz_true );
	    ...
	    */
	    ...
	    ////////////////////////////////////////TESTING USING TEMP VALUES
	    TVector3 zaxis_FP(-sin(SBS_tracker_pitch),0,cos(SBS_tracker_pitch));
	    TVector3 yaxis_FP(0,1,0);
	    TVector3 xaxis_FP = yaxis_FP.Cross(zaxis_FP).Unit();

	    //TVector3 nhat_fp_eth(xpfptemp,ypfptemp,1.0);
	    TVector3 nhat_fp_eth(xpfp_eth,ypfp_eth,1.0);	    
	    nhat_fp_eth = nhat_fp_eth.Unit();

	    TVector3 nhat_fp_global_eth = nhat_fp_eth.X()*xaxis_FP +
	      nhat_fp_eth.Y()*yaxis_FP +
	      nhat_fp_eth.Z()*zaxis_FP;

	    //TVector3 nhat_tgt_eth(xptartemp,yptartemp,1.0);
	    TVector3 nhat_tgt_eth(xptar_eth,yptar_eth,1.0);
	    nhat_tgt_eth = nhat_tgt_eth.Unit();
	    
	    double thetabend_eth = acos( nhat_tgt_eth.Dot(nhat_fp_global_eth) );

	    //precon in this case is pp_etheta_c, which is the proton momentum determined by using the electron kinematics, with central target vertex assumption
	    //precon is needed to define gamma

	    //double pp_thetabend = T->ev_np; //used to test with temp values
	    double pp_thetabend = pp_etheta_c;

	    TVector3 bend_axis_eth = nhat_tgt_eth.Cross(nhat_fp_global_eth).Unit();

	    //double gamma_eth = sqrt(1.0 + pow(pp_etheta_c/Mp,2));
	    double gamma_eth = sqrt(1.0 + pow(pp_thetabend/Mp,2));

	    double chi_eth = gamma_eth*kappa_p*thetabend_eth;

	    double thetaspin_eth = chi_eth + thetabend_eth;
	    //double thetaspin_eth = chi_eth;

	    double theta_bend_temp = (atan(xptartemp) - atan(xpfptemp));


	    hdtheta_bend->Fill((thetabend_eth*57.3) - (theta_bend_temp*57.3));

	    htheta_bend_eth->Fill(thetabend_eth*57.3);
	    htheta_bend_temp->Fill(theta_bend_temp*57.3);
	    
	    
	    hthetaspin_eth->Fill(thetaspin_eth*57.3);
	    hchispin_eth->Fill(chi_eth*57.3);

	    TRotation Rdipole;
	    Rdipole.Rotate( thetaspin_eth, bend_axis_eth );
	    ...
	    TMatrixD Rot_di(3,3);
	    Rot_di(0,0) = Sxx_sum; Rot_di(0,1) = Sxy_sum; Rot_di(0,2) = Sxz_sum;
	    Rot_di(1,0) = Syx_sum; Rot_di(1,1) = Syy_sum; Rot_di(1,2) = Syz_sum;
	    Rot_di(2,0) = Szx_sum; Rot_di(2,1) = Szy_sum; Rot_di(2,2) = Szz_sum;

	    //cout << Rot_c(0,0) << " " << Rot_c(1,0) << " " << Rot_c(2,0) << endl;
	    
	    TVectorD Ptg_di(3); //are these in the correct coordinates to compare????????
	    Ptg_di(0) = Pxtg;//transport coordinates
	    Ptg_di(1) = Pytg;
	    Ptg_di(2) = Pztg;

	    //cout << Ptg(0) << " " << Ptg(1) << " " << Ptg(2) << endl;
	    /*
	    TVectorD Pfp(3);
	    Pfp(0) = 0;
	    Pfp(1) = 0;
	    ...
	    */
	    //Pfp = Rot_c*Ptg;
	    TVectorD Pfp_di = Rot_di*Ptg_di; 
	    double Pxfp_eth_di = Pfp_di(0);
	    double Pyfp_eth_di = Pfp_di(1);
	    double Pzfp_eth_di = Pfp_di(2);
	    
	    hPxfp_di->Fill(Pxfp_eth_di);
	    hPyfp_di->Fill(Pyfp_eth_di);
	    hPzfp_di->Fill(Pzfp_eth_di);



	    hSxx_di->Fill( Sxx_sum );
	    hSxy_di->Fill( Sxy_sum );
	    hSxz_di->Fill( Sxz_sum );

	    hSyx_di->Fill( Syx_sum );
	    hSyy_di->Fill( Syy_sum );
	    hSyz_di->Fill( Syz_sum );

	    hSzx_di->Fill( Szx_sum );
	    hSzy_di->Fill( Szy_sum );
	    hSzz_di->Fill( Szz_sum );

	    double det_di;

	    det_di = Rot_di.Determinant();

	    hDeterminant->Fill(det_di);
		...
		for( int k=0; k<=order_spin-i-j; k++ ){
		  for( int l=0; l<=order_spin-i-j-k; l++ ){
		    for( int m=0; m<=order_spin-i-j-k-l; m++ ){
		      double term_eth = pow(xptartemp,m)*pow(yptartemp,l)*pow(xtartemp,i)*pow(ytartemp,k)*pow(1.0/pp_thetabend,j);
		    
		      //double term_eth = pow(xptar_eth,m)*pow(yptar_eth,l)*pow(xtar_eth,i)*pow(ytar_eth,k)*pow(1.0/pp_thetabend,j);

		      
		      for(int exp_check = 0; exp_check <= Cspinexpon.size(); exp_check++){

			if( Cspinexpon[exp_check][0] == m && Cspinexpon[exp_check][1] == l && Cspinexpon[exp_check][2] == k && Cspinexpon[exp_check][3] == j && Cspinexpon[exp_check][4] == i){

			  
			  Sxx_sum += Cxx[exp_check] * term_eth;
			  Syx_sum += Cyx[exp_check] * term_eth;
			  Szx_sum += Czx[exp_check] * term_eth;
			  
			  Sxy_sum += Cxy[exp_check] * term_eth;
			  Syy_sum += Cyy[exp_check] * term_eth;
			  Szy_sum += Czy[exp_check] * term_eth;
			  
			  Sxz_sum += Cxz[exp_check] * term_eth;
			  Syz_sum += Cyz[exp_check] * term_eth;
			  Szz_sum += Czz[exp_check] * term_eth;
	    ...
	    //cout << "Sxx_sum " << Sxx_sum << endl;
	    
	    TMatrixD Rot_c(3,3);
	    Rot_c(0,0) = Sxx_sum; Rot_c(0,1) = Sxy_sum; Rot_c(0,2) = Sxz_sum;
	    Rot_c(1,0) = Syx_sum; Rot_c(1,1) = Syy_sum; Rot_c(1,2) = Syz_sum;
	    Rot_c(2,0) = Szx_sum; Rot_c(2,1) = Szy_sum; Rot_c(2,2) = Szz_sum;

	    //cout << Rot_c(0,0) << " " << Rot_c(1,0) << " " << Rot_c(2,0) << endl;
	    
	    TVectorD Ptg(3); //are these in the correct coordinates to compare????????
	    Ptg(0) = Pxtg;//transport coordinates
	    Ptg(1) = Pytg;
	    Ptg(2) = Pztg;
	    ...
	    */
	    //Pfp = Rot_c*Ptg;
	    TVectorD Pfp = Rot_c*Ptg; 
	    double Pxfp_eth_c = Pfp(0);
	    double Pyfp_eth_c = Pfp(1);
	    double Pzfp_eth_c = Pfp(2);

	    TVector3 Pfp_eth_c_unit(Pxfp_eth_c,Pyfp_eth_c,Pzfp_eth_c);
	    Pfp_eth_c_unit = Pfp_eth_c_unit.Unit();
	    
	    hPxfpdiff->Fill(Pfp_eth_c_unit.X()-Pxfp);
	    hPyfpdiff->Fill(Pfp_eth_c_unit.Y()-Pyfp);
	    hPzfpdiff->Fill(Pfp_eth_c_unit.Z()-Pzfp);

	    hPxfp_eth_c_vs_true->Fill(Pxfp,Pfp_eth_c_unit.X());
	    hPyfp_eth_c_vs_true->Fill(Pyfp,Pfp_eth_c_unit.Y());
	    hPzfp_eth_c_vs_true->Fill(Pzfp,Pfp_eth_c_unit.Z());

	    //hchi_eth_chi_mc_diff->Fill((chi_eth*57.3) - ());


	    hPxfp_eth_c_vs_chitheta->Fill(chi_eth*57.3,Pfp_eth_c_unit.X());
	    //hPxfp_eth_c_vs_chiphi->Fill(pphitrue,Pxfp_eth_c);
	    ...
	    det = Rot_c.Determinant();

	    hDeterminant->Fill(det);

	    hPxfp_c->Fill(Pxfp_eth_c);
	    hPyfp_c->Fill(Pyfp_eth_c);
	    hPzfp_c->Fill(Pzfp_eth_c);
	    ...
	    ////////////////////////////////////END OF TESTING USING TEMP VALUES



	    ///////////////////////////////////following calculation gives zt or z-true quantities where we no longer assume a central target vertex, but we actually calculate what the correct vertex position in the simulation is and use that instead of any other assumption///////////////////////////////////////////////////////////////////////////////////////
	    ...
	    //Now reconstruct SBS fp quantities from electron arm quantities assuming vertex position is known:              zt quantities
	    //no longer assuming central target vertex like in previous calculation 
	    ...
	    hdxfp_vtx_known_z->Fill(  T->ev_vz, xfp_eth_zt - xfptemp, weight  );
	    hdyfp_vtx_known_z->Fill( T->ev_vz, yfp_eth_zt - yfptemp, weight  );
	    hdxpfp_vtx_known_z->Fill( T->ev_vz, xpfp_eth_zt - xpfptemp, weight  );
	    hdypfp_vtx_known_z->Fill( T->ev_vz, ypfp_eth_zt - ypfptemp, weight  );
	    ...
	    ////////////////////////////////////////////////////////////////////////////////////////////SPIN USING VTX KNOWN QUANTITIES
...
*/
	    ...
	    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////END OF SPIN USING VERTEX QUANTITIES
	    ...
	    ////////////////////////////////////////////////////BEGINNING ELECTRON RECONSTRUCTION USING 
	    ...
	    //reconstructed ecal variables of electron calculated from proton angles only, no momentum
	    ...
	    //reconstructed ecal variables of electron calculated from proton momentum and angle phi only, no theta
	    ...
	    hdxfp_vs_pmisse->Fill( (pprecon - pp_etheta_recon)/ptrue, xfp_eth - xfptemp, weight );
